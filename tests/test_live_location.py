...
def _message(user=USER, **kwargs) -> Message:
    msg = Message(message_id=1, date=None, chat=CHAT, from_user=user, **kwargs)
    msg.set_bot(MagicMock())
    return msg


def _edited_location(lat=1.5, lon=2.5, live_period=None, topic_id=None, user=USER) -> Update:
    return Update(
        update_id=1,
...
def _new_location(lat=1.5, lon=2.5, live_period=None, user=USER) -> Update:
    return Update(
        update_id=1,
        message=_message(
            user=user, location=Location(latitude=lat, longitude=lon, live_period=live_period)
        ),
    )
...
async def test_missing_live_period_keeps_previous_expiry(telegram_interface):
    """A live update that does not restate the period must not clear the known expiry."""
    state = _state(users={"7": {"live_until": 12345.0}})
    saved = await _record(
        telegram_interface, state, "7", LocationContent(latitude=1.0, longitude=1.0)
    )

    assert _stored(saved[0]).live_until == 12345.0
