...
def test_init_does_not_create_memory_md(tmp_path: Path) -> None:
    memory_dir = tmp_path / "memory"
    with patch("aug.core.memory.MEMORY_DIR", memory_dir):
        init_memory_files()
...
def test_init_context_md_has_sections(tmp_path: Path) -> None:
    memory_dir = tmp_path / "memory"
    with patch("aug.core.memory.MEMORY_DIR", memory_dir):
        init_memory_files()
...
def test_init_context_md_has_timestamp_format(tmp_path: Path) -> None:
    memory_dir = tmp_path / "memory"
    with patch("aug.core.memory.MEMORY_DIR", memory_dir):
        init_memory_files()

    content = (memory_dir / "context.md").read_text()
    # Default template must demonstrate the [YYYY-MM-DD] entry format
