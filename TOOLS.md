...
| `run_bash` | ✅ Done | Full shell access inside the Docker container. Wraps command with `hushed run --` for automatic secret injection + redaction. Blocked commands configurable via `data/bash_blacklist` (regex, empty by default). |
